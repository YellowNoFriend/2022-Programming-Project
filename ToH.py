...
class Move(object):
    """
    The class Move as introduced in the document. We interpret this as a move in the towers of Hanoi.
...
def ToH_Basic(n, speed_factor):
    ...
    TODO: Add a step at the end to show the animation.
    """
    ...
    run_anim = False
    # Main Loop
    while running and index_move != len_move_list - 1:
        # Key Press Detecting
        ...
        if run_anim:
            ...
            # Update iterator
            index_move = (index_move + 1) % len_move_list
...
def ToH_NoWrap(n, speed_factor):
    ...
    screen = pygame.display.set_mode(screen_size)
    # Title bar
    caption = 'Tower of Hanoi - No Wrap Variant'
    caption += '                              '
    caption += '(Keystroke:  \'Space\' to start or pause)'
    pygame.display.set_caption(caption)
    # Draw the background
...
def ToH_Animation_Interactive(): 
    ...
    while not GameEnd:
        ...
        try:
            if Mode == 'c':
                n = input('start with how many disk? ')
                sf = input('Give me a speed factor ')
                ToH_Basic(int(n), int(sf))
                GameEnd = True
            elif Mode == 'n':
                n = input('start with how many disk? ')
                sf = input('Give me a speed factor ')
                ToH_NoWrap(int(n), int(sf))
                GameEnd = True
            elif Mode == 'b':
                n = input('start with how many disk? ')
                sf = input('Give me a speed factor ')
                ToH_BiColor(int(n), int(sf))
                GameEnd = True
            elif Mode == 'q':
...
def ToH_Basic_Smooth(n, speed_factor):
    ...
    TODO: Add a step at the end to show the animation.
    TODO: General bug fixes, and if have time, move up disk thing
    """
    x_step = 10
    y_step = 5
    # Basic initialization
